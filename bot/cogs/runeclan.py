...
class RuneclanTracker(commands.Cog):
    ...
    def format_tracker(self, data: str) -> ExpTracker:
        ...
        attrs: Dict[str, str] = {'class': 'regular', 'width': '100%', 'cellpadding': '0', 'cellspacing': '0'}
    ...
    async def exp_tracker(self, ctx: commands.Context, inicio: str, fim: str, habilidade: str = 'geral', quantidade: int = 10):
        skills = {
            'geral': {
                'id': 2,
                'emoji': '<:overall_skill:647655515266088986>'
            },
            'ataque': {
                'id': 3,
                'emoji': '<:attack:499707565949583391>'
            }
        }
        ...
        if not skill:
            ...
            return await ctx.send(f'Habilidade \'{habilidade}\' não encontrada. Opções:\n\n{skill_options}')

        url: str = f'http://www.runeclan.com/clan/Atlantis/xp-tracker?skill={skill["id"]}'
        ...
        embed.set_thumbnail(url=f'http://services.runescape.com/m=avatar-rs/{self.bot.setting.clan_name}/clanmotif.png')
