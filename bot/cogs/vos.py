...
from atlantisbot_api.models import VoiceOfSeren
...
class Vos(commands.Cog):
    ...
    def vos_embed(self, vos_1: str, vos_2: str) -> Tuple[discord.Embed, discord.File]:
        ...
        embed = discord.Embed(
            title="Voz de Seren", description=text, color=discord.Color.blue()
        )
    ...
    async def vos_update(self, ctx: Context):
        state = VoiceOfSeren.object()
        channel: discord.TextChannel = self.bot.get_channel(
            self.bot.setting.chat.get("vos")
        )
        if state:
            ...
            if voices:
                ...
                if (
                    vos_1 != state.current_voice_one
                    and vos_2 != state.current_voice_two
                ):
                    try:
                        message: Optional[
                            discord.Message
                        ] = await channel.fetch_message(int(state.message_id))
            ...
            if voices:
                ...
                vos_message: discord.Message = await channel.send(
                    file=file, embed=embed, content=""
                )
                await channel.send(content=mentions, delete_after=5 * 60)
                state = VoiceOfSeren(
                    current_voice_one=vos_1,
                    current_voice_two=vos_2,
                    message_id=str(vos_message.id),
                )
                state.save()
    ...
    async def change_vos(
        ...
        new_message: discord.Message = await channel.send(
            embed=embed, file=file, content=mentions
        )
    ...
    async def voice_of_seren(self, ctx: Context):
        ...
        if voices:
            ...
            return await ctx.send(
                "Erro ao pegar dados da Voz de Seren. Tente novamente mais tarde."
            )
