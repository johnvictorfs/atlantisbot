...
def get_competitions(clan):
    ...
    comp_count = soup.find(
        "span", attrs={"class": "events_comp_num", "style": "color:green;"}
    )
    finished_count = soup.find(
        "span", attrs={"class": "events_comp_num", "style": "color:purple;"}
    )
...
class Competitions(commands.Cog):
        ...
        if len(competitions["running_competitions"]) > 1 and index == 0:
            ...
            for comp in competitions["running_competitions"]:
                ...
                if comp["start_date"] == "active":
                    field_value += (
                        f"__Tempo Restante__: {translate(comp['time_remaining'])}"
                    )
                else:
                    field_value += (
                        f"__Início daqui a__: {translate(comp['start_date'])}"
                    )
                field_value += "\n"
            ...
            if competition["start_date"] == "active":
                comp_details = competition_details(
                    self.bot.setting.clan_name, competition["link"]
                )
                if comp_details == "FAILURE":
                    await ctx.send(
                        "Erro ao conectar ao RuneClan. Tente novamente mais tarde."
                    )
                comp_embed.add_field(
    ...
    async def comp_pontos(self, ctx: Context, number=10):
        ...
        url = url.format(
            key="1iHPQovW4NXFicJd6ot83QnrN9NyLlxcX3UraJHv9uPg", sheet_name="min"
        )
        with closing(requests.get(url, stream=True)) as r:
            ...
            comp_embed = discord.Embed(
                ...
                "2PACX-1vRS1xBkGJi6G5utxcbHJRkKxum2qmcKdvLv7A-O4bFKvnujF_pOSK0tps5gZU1MjSkIbEY-Bup5fJDm/pubhtml#",
            )
