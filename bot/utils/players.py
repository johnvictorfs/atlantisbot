...
async def get_player_df_runeclan(player_name: str) -> pd.DataFrame:
    ...
    player_skills = [
        {headers[i]: cell.text for i, cell in enumerate(row.find_all('td')) if headers[i] in value_headers}
    ...
    player_skills = [
        {new_value_headers[key]: value.replace(',', '') for key, value in skill.items()} for skill in player_skills
    ]
