...
class Bot(commands.Bot):
    def __init__(self, logger: logging.Logger, intents: discord.Intents):
        ...
        self.api = api.BotApi(
            base_url=self.setting.rsatlantis["API_URL"],
            api_token=self.setting.rsatlantis["API_TOKEN"],
        )
    ...
    async def send_logs(self, e, tb, ctx: context.Context, more_info: object = None):
        ...
        if ctx:
            info_embed = discord.Embed(
                title="__Error Info__", color=discord.Color.dark_red()
            )

            info_embed.add_field(
                name="Message", value=ctx.message.content, inline=False
            )
        ...
        if more_info:
            extra_embed = discord.Embed(
                title="__Extra Info__", color=discord.Color.dark_red()
            )
    ...
    def setting(self) -> settings.Settings:
        try:
            ...
            with open("bot/bot_settings.json", "w+") as f:
                ...
                print(
                    f"{colorama.Fore.YELLOW}Settings not found. Default settings setup."
                    "Edit 'bot/bot_settings.json' to change settings, then reload."
                )
    ...
    async def on_ready(self):
        ...
        await self.change_presence(
            activity=discord.Game(name=self.setting.playing_message)
        )
    ...
    async def on_message(self, message: discord.Message):
        ...
        if self.setting.mode == "dev":
            if not message.guild:
                ...
                message.guild.id != self.setting.dev_guild
    ...
    async def on_member_remove(self, member: discord.Member):
        ...
        embed = discord.Embed(
            title="Saiu do Servidor", description="\u200B", color=discord.Color.red()
        )
        ...
        embed.add_field(
            name="Cargos", value=roles.replace("@everyone, ", "").replace("ﾠ", "")
        )
